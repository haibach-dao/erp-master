...
describe('phạm vi — method nhận Caller thì phải HỎI phạm vi', () => {
  ...
  it('không có lỗ MỚI nào ngoài bốn chỗ đã đo và đã nêu để quyết', () => {
    ...
    expect(
      unexpected,
      'Method này nhận `Caller` nhưng không gọi assertCompanyFor/assertPlotFor/plotScopeFilterFor/visibleCompanyIdsFor/listSiteFilterFor/levelFor (kể cả qua helper). Gate quyền không gate BẢN GHI: hãy bó phạm vi, hoặc thêm vào MEASURED_UNGUARDED kèm lý do.',
    ).toEqual([]);
  });
